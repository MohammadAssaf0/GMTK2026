    ...
    void OnControllerColliderHit(ControllerColliderHit hit) 
{
    ...
    // Only push objects with a Rigidbody and avoid pushing objects that are kinematic
    if (body == null || body.isKinematic) 
        return;

    // Calculate the push direction from the character's movement direction
    Vector3 pushDir = new Vector3(hit.moveDirection.x, 0, hit.moveDirection.z);
    
    // Apply the force to the Rigidbody
    // Adjust the force multiplier as needed to balance the push strength
